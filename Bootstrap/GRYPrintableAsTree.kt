...
class GRYPrintableTree: GRYPrintableAsTree {
	companion object {
		internal fun initialize(description: String, subtreesOrNil: MutableList<GRYPrintableAsTree?>): GRYPrintableTree? {
...
public fun GRYPrintableAsTree.prettyPrint(indentation: MutableList<String> = mutableListOf(), isLast: Boolean = true, horizontalLimit: Int = Int.MAX_VALUE, printFunction: (String) -> Unit = { print(it) }) {
	...
	for (subtree in subtrees.dropLast(1)) {
		...
		newIndentation.add(" ├─")
		subtree.prettyPrint(indentation = newIndentation, isLast = false, horizontalLimit = horizontalLimit, printFunction = printFunction)
	}
	val newIndentation: MutableList<String> = indentation.copy()
	newIndentation.add(" └─")
	subtrees.lastOrNull()?.prettyPrint(indentation = newIndentation, isLast = true, horizontalLimit = horizontalLimit, printFunction = printFunction)
}
