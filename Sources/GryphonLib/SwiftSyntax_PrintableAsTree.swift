...
extension SyntaxProtocol {
    ...
    return SwiftSyntaxToPrintableTreeVisitor(viewMode: .sourceAccurate).convertToPrintableTree(Syntax(self))
	}
...
open class SwiftSyntaxToPrintableTreeVisitor: SyntaxVisitor {
    ...
    let children = node.children(viewMode: .sourceAccurate).map { visit($0) }

		subtrees.append(contentsOf: children)

    let name = node.debugDescription
...
extension Syntax {
	func getName() -> String? {
    // Don't know if that works - it should but not sure yet.
    String(describing: self.syntaxNodeType)
...
//
//    #if swift(>=5.8)
//    if self.is(MissingSyntax.self)
//    { return "Unknown" }
//    else if self.is(MissingDeclSyntax.self)
//    { return "Unknown Declaration" }
//    else if self.is(MissingExprSyntax.self)
//    { return "Unknown Expression" }
//    else if self.is(MissingStmtSyntax.self)
//    { return "Unknown Statement" }
//    else if self.is(MissingTypeSyntax.self)
//    { return "Unknown Type" }
//    else if self.is(MissingPatternSyntax.self)
//    { return "Unknown Pattern" }
//    #else
//    if self.is(UnknownSyntax.self)
//    { return "Unknown" }
